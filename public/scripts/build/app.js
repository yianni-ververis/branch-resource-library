  	...
  	function linkFunction($scope, $element, $attrs) {
  		...
  		function updateIt(element, charIndex, arrIndex, text) {
  			if (charIndex <= text.length) {
  				element.text(text.substring(0, charIndex) + cursor);
  				charIndex++;
  				timer = $timeout(function () {
  		...
  		function blinkIt(element, charIndex) {
  			...
  			if (blinkCursor) {
  				if (blinkDelay) {
  					...
  					element.text(text.substring(0, charIndex)).append('<span class="blink" style="' + auxStyle + '">' + cursor + '</span>');
  				} else {
  					element.text(text.substring(0, charIndex)).append('<span class="blink">' + cursor + '</span>');
  				}
  			} else {
  				element.text(text.substring(0, charIndex)).html(text.substring(0, charIndex));
  			}
  		}
  		...
  		function cleanAndRestart(element, charIndex, arrIndex, currentText) {
  			if (charIndex > 0) {
  				...
  				element.text(currentText + cursor);
  				charIndex--;
  				timer = $timeout(function () {
